...
class MolecularData:
    ...
    def _load_data(cls) -> 'MolecularData':
        ...
        vdw_radii = {element: vdw_data[element]['vdw_radius'] * BOHR_TO_ANGSTROM
                     for element in vdw_data if not element.startswith('_')}
        ...
        try:
            ...
            expected_valences = {element: valence_data[element] 
                                for element in valence_data if not element.startswith('_')}
        ...
        try:
            ...
            with ve_file.open('r') as f:
                ve_data = json.load(f)
            valence_electrons = {el: ve_data[el] 
                                for el in ve_data if not el.startswith('_')}
        except Exception as e:
