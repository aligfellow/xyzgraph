...
class GraphBuilder:
    ...
    def _validate_bond_geometry(self, G: nx.Graph, i: int, j: int, distance: float, confidence: float, baseline_bonds=None) -> bool:
        """
        ...
        for existing_neighbor in G.neighbors(i):
            ...
            if angle > 160.0:
                ...
                if has_metal:
                    self.log(f"  Bond {i}-{j}: collinear ({angle:.1f}°) with {existing_neighbor}-{i}, involves metal ({sym_i}-{sym_j}) - allowed", 4)
                    continue
    ...
    def _build_initial_graph(self) -> nx.Graph:
        ...
        for confidence, i, j, d, has_metal in baseline_bonds:
            ...
            self.log(f"  Evaluating bond {si}{i}-{sj}{j} (d={d:.3f} Å, conf={confidence:.2f})", 3)
...
def build_graph_rdkit(
    ...
    try:
        ...
        if any(s in DATA.metals for s in symbols):
            raise ValueError(
                f"RDKit DetermineBonds failed (metal atoms detected): {e}"
            )
...
def build_graph_rdkit_tm(
    ...
    if isinstance(xyz_file, str):
        ...
        atoms = xyz_file
        if bohr_units:
            atoms = [(s, (x*BOHR_TO_ANGSTROM, y*BOHR_TO_ANGSTROM, z*BOHR_TO_ANGSTROM))
    ...
    for bond in mol.GetBonds():
        ...
        bo = {Chem.BondType.SINGLE: 1.0, Chem.BondType.DOUBLE: 2.0, 
        ...
        G.add_edge(
            ...
            metal_coord=(G.nodes[i_xyz]['symbol'] in DATA.metals or 
                        G.nodes[j_xyz]['symbol'] in DATA.metals)
        )
    ...
    for idx, (sym, pos) in enumerate(atoms):
        if sym == 'H':
            ...
            dists = [np.linalg.norm(pos_arr - np.array(G.nodes[i]['position'])) 
                    for i in heavy_idx]
    ...
    for node in G.nodes():
        G.nodes[node]['valence'] = sum(G.edges[node, nbr]['bond_order'] 
                                       for nbr in G.neighbors(node))
    ...
    # --- Metadata ---
    from . import __version__, __citation__
    G.graph['metadata'] = {'version': __version__, 'citation': __citation__, 
                          'source': 'rdkit_tm'}
    G.graph['total_charge'] = charge
