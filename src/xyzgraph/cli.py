...
def display_graph(G, args, show_h_indices, label=""):
    ...
    if show_ascii:
        ...
        print(f"\n{'=' * 80}\n{title}\n{'=' * 80}\n")
        ascii_out = graph_to_ascii(G, scale=max(0.2, args.ascii_scale), 
...
def main():
    ...
    p.add_argument("-as", "--ascii-scale", type=float, default=2.5, help="ASCII scaling factor (default: 2.5)")
    p.add_argument("-H", "--show-h", action="store_true", help="Include hydrogens in visualizations")
    p.add_argument("--show-h-idx", type=str, help="Show specific hydrogen atoms (comma-separated, e.g., '3,7,12')")
    ...
    # xTB specific
    p.add_argument("--no-clean", action="store_true", help="Keep temporary xTB files (only for --method xtb)")
    ...
    metadata = compute_metadata(
        method=args.method, charge=args.charge, multiplicity=args.multiplicity,
        ...
        bond=bond, unbond=unbond, clean_up=not args.no_clean,
